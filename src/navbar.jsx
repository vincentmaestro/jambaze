...
const Navbar = () => {
    ...
    function darkMode(e) {
        ...
        else if(e.target.classList.contains('dark-mode-mobile')) {
            document.querySelector(':root').style.colorScheme = 'dark';
            document.querySelector('.open-nav').style.display = 'block';
            document.querySelector('.navlinks').classList.remove('nav-open');
            document.querySelector('.close-nav').style.display = 'none';
            document.querySelector('.light-mode-mobile').style.display = 'block';
            document.querySelector('.dark-mode-mobile').style.display = 'none';
        }
    }
    ...
    function lightMode(e) {
        ...
        else if(e.target.classList.contains('light-mode-mobile')) {
            document.querySelector(':root').style.colorScheme = 'light';
            document.querySelector('.open-nav').style.display = 'block';
            document.querySelector('.navlinks').classList.remove('nav-open');
            document.querySelector('.close-nav').style.display = 'none';
            document.querySelector('.light-mode-mobile').style.display = 'none';
            document.querySelector('.dark-mode-mobile').style.display = 'block';
        }
    }
    ...
    return (
        <nav className="flex items-center justify-center py-5 tablet:gap-x-[3%] tablet_s:justify-between tablet_s:px-[5%] tablet_s:py-3 tablet_s:sticky tablet_s:top-0 z-10 tablet_s:bg-stone-600">
            ...
            <ul className="navlinks w-[65%] flex justify-center gap-x-[4%] tablet:w-[55%] tablet_s:bg-stone-600 tablet_s:block tablet_s:fixed tablet_s:w-[44%] tablet_s:text-center tablet_s:py-4 tablet_s:rounded-xl tablet_s:top-[2%] tablet_s:right-[-100%] transition-all duration-300 mobile:w-[54%] mobile_m:w-[70%]">
                <li className="cursor-pointer tablet_s:mb-2 text-xl tablet:text-lg tablet_s:text-white hover:text-orange-400" onClick={closeNav}><a href="#footer">About us</a></li>
                <li className="cursor-pointer tablet_s:mb-2 text-xl tablet:text-lg tablet_s:text-white hover:text-orange-400" onClick={closeNav}>Blog</li>
                <li className="cursor-pointer tablet_s:mb-2 text-xl tablet:text-lg tablet_s:text-white hover:text-orange-400" onClick={closeNav}>Community</li>
                <li className="cursor-pointer tablet_s:mb-2 text-xl tablet:text-lg tablet_s:text-white hover:text-orange-400" onClick={closeNav}><a href="#footer">Contact us</a></li>
                <li className="cursor-pointer rounded-[8px] text-2xl text-white hidden tablet_s:block dark-mode-mobile" onClick={darkMode}><i className="fa-solid fa-moon" /> Dark mode</li>
                <li className="cursor-pointer rounded-[8px] text-2xl text-white hidden light-mode-mobile" onClick={lightMode}><i className="fa-regular fa-sun" /> Light mode</li>
            </ul>
            <button className="rounded-[8px] bg-orange-500 text-white px-[8px] py-[5px] hover:bg-slate-800 transition-colors duration-200 tablet_s:hidden dark-mode" onClick={darkMode}><i className="fa-solid fa-moon" /> Dark mode</button>
            <button className="rounded-[8px] px-[8px] py-[5px] hover:bg-white hover:text-black transition-colors duration-200 hidden tablet_s:hidden light-mode" onClick={lightMode}><i className="fa-solid fa-sun" /> Light mode</button>
